...
IDEA ──► DESIGN ──► CODE ──► REVIEW ──► CI ──[MERGE]──► STAGING ──► CANARY ──► PROD
  │         │         │         │         │                              │          │
  ▼         ▼         ▼         ▼         ▼                              ▼          ▼
Require-  Threat    TDD /     Contract  Static                        Canary     Synthetic
ments     modelling  BDD       tests     analysis                     + SLO      monitoring
review    (STRIDE)   loops     (pre-push) lint                        gates      (every 5 min)
  │                                                                               │
  └─────────────────── PRE-MERGE (shift-left moves) ───────────────────────────┘
                                            └─── POST-MERGE (shift-right moves) ──┘

Cheapest bug: requirements review         Most valuable bug: canary rollback
Most commonly skipped: requirements       Most commonly skipped: synthetic alerts
```
</Diagram>
