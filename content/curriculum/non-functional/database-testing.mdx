...
title: Database Testing with Testcontainers + Drizzle
...
**Prerequisites:** Node ≥ 22.12.0, Docker Desktop, a terminal.
...
npm install --save-dev @testcontainers/postgresql@12.0.1 testcontainers@12.0.1 \
  drizzle-kit@0.31.10 vitest@4.1.7 tsx@4.19.4
```
...
beforeAll(async () => {
  ...
  client = postgres(url);
  db = drizzle(client);
}, 60_000); // allow container startup
...
```
 ...
 ✓ tests/integration/constraints.test.ts (2 tests) 6.3s

 Test Files  1 passed (1)
 Tests  2 passed (2)
 Duration  6.3s (container start + migration + tests)
```

If both tests pass, the setup is complete.
...
```
TESTCONTAINERS LIFECYCLE (per test suite)
  ┌─────────────────────────────────────────────────────────┐
  │ beforeAll: PostgreSqlContainer('postgres:16').start()   │
  │   → pulls image (first run only) → starts container    │
  │   → exposes random host port → returns connectionUri   │
  ├─────────────────────────────────────────────────────────┤
  │ runMigrations(url)  — drizzle migrate()                 │
  │   → applies every SQL file in drizzle/ in order        │
  │   → schema is identical to production schema            │
  ├─────────────────────────────────────────────────────────┤
  │ TESTS RUN  (isolated, real Postgres constraints)        │
  ├─────────────────────────────────────────────────────────┤
  │ afterAll: container.stop()  → container + data removed  │
  └─────────────────────────────────────────────────────────┘
  Key property: each test suite starts from a clean schema.
  Shared dev DB → tests develop ordering dependencies.
  Testcontainers → each suite is fully isolated.

EXPAND-CONTRACT MIGRATION (safe column rename)
GOAL: rename users.email_address → users.email
  ...
  │   WHERE email IS NULL;   (batch; 1 000 rows at a time)  │
  ...
  Anti-pattern (single-step rename):
    ALTER TABLE users RENAME COLUMN email_address TO email;
    → table-lock for entire operation (hours on 100 M rows)
...
test('atomic update prevents lost-update race', async () => {
  ...
  const [after] = await db.select({ balance: accounts.balance })
    .from(accounts).where(sql`id = ${row.id}`);
...
<Maintain trigger="Testcontainers ships a new major version (e.g. 12.x → 13.x)">
  1. Read the changelog for breaking API changes (particularly `start()` return type and connection URI method names — these have changed between majors).
...
<Maintain trigger="Drizzle ORM or drizzle-kit ships a breaking schema-generation change">
  1. Run `npx drizzle-kit generate` on the current schema — compare new output against existing migration files.
...
<Maintain trigger="Docker Desktop upgrades and containers fail to start (image pull or socket error)">
  ...
  2. On Windows, verify the WSL2 backend is active and `DOCKER_HOST` is not overriding the socket path.
  3. Run `docker pull postgres:16-alpine` manually to confirm the image is pullable.
  4. If using a pinned image digest, update to the current digest for `postgres:16-alpine`.
  5. Re-run `beforeAll` with a longer timeout (`120_000`) if the pull adds latency on cold CI runners.
</Maintain>

<Maintain trigger="A migration fails on production (table lock or constraint violation on existing rows)">
  1. Identify whether the migration is a single-step operation (rename, NOT NULL add, index create) — these are the common lock sources.
