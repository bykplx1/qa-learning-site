...
```
DECISION AXIS: Who reads the test results?

  Engineering only          Mixed audience             Non-engineering required
  ─────────────────         ─────────────────          ─────────────────────────
  Tests-as-code +           Hybrid pattern:            Full TMT:
  CI reporter +             tests-as-code              TestRail / qTest /
  Slack summary             → CI → TMT API             Zephyr / Xray
                            → dashboard                + manual case library
                                                       + traceability matrix

  Case volume: < 200        200 – 2 000                > 500 + multi-env
  Audit: none               optional hygiene           regulatory / SOC 2

──────────────────────────────────────────────────────────────────────────────

MODERN HYBRID DATA FLOW

  .spec.ts files            CI pipeline               TMT (e.g. TestRail)
  ┌──────────────┐          ┌────────────┐            ┌───────────────────┐
  │ @C12345      │──run──►  │ reporter   │──API───►   │ run record        │
  │ @C12346      │          │ plugin     │            │ pass/fail/trend   │
  └──────────────┘          └────────────┘            └─────────┬─────────┘
                                                                │
  Manual cases (pre-release non-automatable flows)             ▼
  ────────────────────────────────────────────────    Exec dashboard
                                                      (PM / auditor view)
```
</Diagram>
