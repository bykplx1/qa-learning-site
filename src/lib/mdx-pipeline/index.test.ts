...
describe('MDX pipeline contract', () => {
  ...
  it('has rehypeTakeawayBlockquote as the first rehype plugin', () => {
    expect(REHYPE_PLUGINS[0]).toBe(rehypeTakeawayBlockquote);
  });

  it('contains all expected transforms (4 remark + 1 rehype)', () => {
    expect(REMARK_PLUGINS).toHaveLength(4);
    expect(REHYPE_PLUGINS).toHaveLength(1);
  });
});
