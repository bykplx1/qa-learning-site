...
```
THREE TREES (maintained simultaneously)
──────────────────────────────────────────────────────────────────────
  DOM tree              Render tree           Accessibility tree
  (parser output +      (DOM + CSS layout +   (semantic role layer;
   JS mutations)         paint info)           consumed by AT + getByRole)

  DevTools: Elements    DevTools: Layers/      DevTools: Accessibility
                        Performance            panel (Computed Properties)

  Locator family:       not directly           getByRole / getByLabel /
  css / xpath           queryable              getByText (role-first)

FOUR RENDERING STRATEGIES
──────────────────────────────────────────────────────────────────────
  Strategy   HTML at first byte?   JS required for content?   Tester implication
  ────────   ───────────────────   ────────────────────────   ──────────────────
  CSR        Empty <div id="root"> Yes — wait for hydration   view-source is useless
  SSR        Full content          Yes — hydration binds UX   visible ≠ interactive
  SSG        Full content          Yes — hydration binds UX   same as SSR at runtime
  ISR        Full content          Yes — stale-while-revalidate  cache state matters

HYDRATION WINDOW (SSR / SSG / ISR)
──────────────────────────────────────────────────────────────────────
  First byte arrives ──► HTML painted ──► JS bundle loads ──► hydration ──► interactive
                         │                                      │
                  VISIBLE (test can read DOM)           INTERACTIVE (onClick bound)
                         │◄────── FLAKE ZONE ──────────────────►│
                  clicks here silently do nothing — test assertion times out later
```
</Diagram>
