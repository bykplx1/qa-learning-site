...
```
Oracle Types (Barr et al. 2015)
────────────────────────────────────────────────────────
  Specified  │ Formal spec, contract, state machine
  Derived    │ Snapshot, golden-master, regression baseline
  Implicit   │ No crash, no 5xx, no unhandled exception
  Pseudo     │ Separately-implemented reference (diff testing)
...
★ Critical — test first; document the oracle explicitly
```
</Diagram>
