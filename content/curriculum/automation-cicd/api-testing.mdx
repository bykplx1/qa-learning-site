...
```
LAYER 1 — STATUS ASSERTION (baseline, almost everyone ships this)
...
LAYER 2 — SCHEMA ASSERTION (adds shape validation)

  const validate = ajv.compile(userSchema);
    → expect(status).toBe(200)
    → expect(validate(res.body)).toBe(true)   ← shape check

  CATCHES: wrong types, missing required fields, extra unexpected fields
  MISSES:  shape drift between separately-deployed services

────────────────────────────────────────────────────────────────

LAYER 3 — CONTRACT ASSERTION (consumer-driven, cross-service)

  CONSUMER publishes → BROKER stores → PROVIDER verifies in CI
  ...
  CATCHES: provider change breaks a downstream consumer's actual usage
  MISSES:  nothing the other layers catch — this is the ceiling

────────────────────────────────────────────────────────────────

KEY INSIGHT: escalate in order. A contract test on a status-only suite
amplifies nothing. Schema first, contract second, on routes that cross
a service boundary or a team boundary.
```
</Diagram>
