...
```
AI ATTACK SURFACES & LAYERED DEFENCES
═══════════════════════════════════════

INPUT LAYER          MODEL CORE           OUTPUT LAYER
───────────          ──────────           ────────────
User input ──────→ [System Prompt]  ────→ Text response
                   [Injected chunk]       Tool calls
Retrieved docs ──→ [Tool outputs  ]  ────→ Rendered HTML/MD
Tool results ────→ [Chat history  ]       External actions

FIVE ATTACK SURFACES
────────────────────
① Direct prompt injection     ③ Data leakage
  User input overrides           System prompt extraction
  system instructions            PII echo, cross-tenant leak
  "Ignore prior instructions…"   Training-data regurgitation

② Indirect prompt injection   ④ Output handling
  Poisoned retrieved content      SQL / JS / shell in model output
  Web pages, emails, RAG chunks   Markdown image exfiltration
  Any text the model reads        Rendered content → network calls

                    ⑤ Excessive agency
                       Tools without authZ checks
                       Broad permissions, no human-in-loop
                       Recursive/amplifying tool chains

THE LETHAL TRIFECTA (Simon Willison)
──────────────────────────────────────
  If an agent has ALL THREE legs:
    ① Reads untrusted content (web, docs, emails)
    ② Accesses private data (user records, internal APIs)
    ③ Can communicate externally (send email, POST to URL)
  → One indirect injection = data exfiltration

  Break one leg to defuse the trifecta:
    Remove external-write tools   OR
    Scope private-data access     OR
    Sandbox / allowlist retrieval sources

LAYERED DEFENCE
───────────────
  ┌─────────────────────────────────────────────┐
  │  INPUT VALIDATION  (sanitise, delimit, scope) │
  ...
  │  MONITORING        (refusal spikes, cost spikes, abnormal tool calls) │
  └─────────────────────────────────────────────┘
    No single layer is the boundary — the overlap is.
```
</Diagram>
