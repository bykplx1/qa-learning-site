...
ZAP BASELINE SCAN FLOW (what this runbook sets up)
  docker run zaproxy/zap-stable:2.15.0 zap-baseline.py -t <target>
    Spider   — discovers paths via passive crawl
    Passive  — flags headers, cookies, info disclosure (no active attacks)
    Report   — HTML + JSON artefact with PASS/WARN/FAIL per rule
  Juice Shop (local) used as scan target — deliberately vulnerable, legal to probe
```
</Diagram>
