...
```
Browser ──► CDN ──► App Server ──► Database ──► Cache ──► External API
   │          │           │              │          │            │
   │          │           │              │          │            │
   ▼          ▼           ▼              ▼          ▼            ▼
 UI seam   Network     Process        DB seam    Cache       3rd-party
           seam        boundary                  seam         seam

Test type          │ Seams crossed (real)       │ Seams doubled
───────────────────┼────────────────────────────┼──────────────────────────
Solitary unit      │ none                       │ all non-trivial seams
Sociable unit      │ cheap in-process seams     │ expensive / external seams
DB integration     │ DB seam only               │ network, cache, 3rd-party
Contract test      │ one HTTP seam (verified)   │ both services never co-run
E2E                │ all seams real             │ none

Seams crossed = flake opportunities + feedback latency
Seams doubled = bugs missed at that boundary
```
</Diagram>
