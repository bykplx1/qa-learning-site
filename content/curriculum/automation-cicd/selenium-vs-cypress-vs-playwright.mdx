...
```
SELENIUM (WebDriver / W3C protocol)
──────────────────────────────────────────────────────────────────
  Test process              Browser driver          Browser
  (Java / Python /          (chromedriver /         (Chrome /
  JS / C# / Ruby)           geckodriver /           Firefox /
       │                    safaridriver)           Edge / IE)
       │── HTTP/REST ──────►│─── driver protocol──►│
       │◄── response ───────│◄─────────────────────│
  Out-of-process.  W3C standard.  Protocol-stable.
  No built-in trace. Screenshot / log via driver.
  Parallel: requires Grid or cloud provider.

CYPRESS (in-browser / same JS context)
──────────────────────────────────────────────────────────────────
  Node proxy             Iframe: test runner + app in same origin
  (cy.intercept,    ──── injects ─────────────────────────────►
  webpack bundle)                  │
                           ┌───────┴───────────────────────┐
                           │  Cypress test JS + App JS     │
                           │  same tab · same origin       │
                           │  cy.intercept via XHR hook    │
                           └───────────────────────────────┘
  Trace: Cypress App time-travel replay (screenshots per command).
  Multi-tab: limited. Cross-origin: cy.origin (v12+, with caveats).
  Parallel: Cypress Cloud (paid) or self-orchestrated.

PLAYWRIGHT (CDP/BiDi · out-of-process, in-context)
──────────────────────────────────────────────────────────────────
  Test process            Playwright server           Browser
  (JS/TS / Python /       (out-of-process)           contexts
  Java / .NET)                                       (cheap, isolated)
       │── WebSocket ────►│─── CDP / BiDi ──────────►│  context A
       │                  │                          │  context B  ← parallel
       │                  │  page.route()            │  context C
       │                  │  storageState            │
       │                  └──────────────────────────┘
  Trace: trace.zip — action-by-action time travel (built-in).
  Multi-tab: yes (waitForEvent('page') on popup).
  Cross-origin: yes (separate browser contexts).
  Parallel: free, default.

DECISION AXES AT A GLANCE
──────────────────────────────────────────────────────────────────
  Team language     Java/Ruby/C#/Python? → Selenium or Playwright
                    JS/TS only?          → any, Cypress included
  Multi-tab / OAuth popup?               → Playwright or Selenium
  Mobile-web real device?                → Selenium + Appium
  Long-lived enterprise suite (10+ yrs)? → Selenium (protocol stability)
  Greenfield JS/TS E2E (~2023+)?         → default Playwright
  Dashboard / turnkey reporting?         → Cypress Cloud or BrowserStack
  Parallel in CI at no extra cost?       → Playwright
```
</Diagram>
