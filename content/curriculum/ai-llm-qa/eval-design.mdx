---
slug: eval-design
title: Eval Design with promptfoo — Rubrics, Datasets, CI Tiers & Judge Calibration
cluster: ai-llm-qa
layer: systems
kind: tool
verified:
...
Everything from an empty folder to a first green promptfoo eval run.

**Prerequisites:** Node >= 22.12.0, an OpenAI API key (or any supported LLM provider).
...
```
eval-runbook/
  promptfooconfig.yaml     # eval definition (prompts, providers, tests, assertions)
  data/
    incidents.csv          # input dataset rows
  rubrics/
    incident-title.md      # the written rubric (exists before the dataset)
```

### Write the rubric first

Create `rubrics/incident-title.md` before touching the dataset or config:
...
2. VERB-LEAD     Title starts with an imperative verb (rule-based: regex)
3. SUBJECT-REF   Title references the actual subject of the ticket (model-graded)
4. NO-PII        Title contains no email, phone, or name (rule-based + model-graded)
```
...
tests:
  - vars:
      ticket: "The database connection pool is exhausted under peak load; users see 504 errors."
    assert:
      - type: javascript
  ...
  - vars:
    ...
    assert:
      ...
      - type: llm-rubric
        value: "Does the title avoid mentioning any email address or customer name? Answer only 'pass' or 'fail'."
...
───────────────────────
Output type          → Recommended modality        → promptfoo assertion type
─────────────────────────────────────────────────────────────────────────────
Format compliance    → Rule-based                  → javascript / not-contains / regex
Classification /     → Reference-based             → similar-to / levenshtein / cost
  extraction /
  translation
Open-ended           → Model-graded                → llm-rubric / model-graded
  generation /
  conversation /
  faithfulness
...
─────────────────────────────────────────────────
  ...
  At N=50, most LLM prompt-optimisation gains are inside the noise floor.
```
</Diagram>
...
Add rows to `data/incidents.csv` and reference via `tests` in `promptfooconfig.yaml`:
...
Before trusting `llm-rubric` scores, run a calibration pass: designate 10–20 rows, grade them yourself, then compare to the judge. Compute Cohen's kappa — anything below 0.6 means the rubric prompt is ambiguous. Fix the rubric prompt, not the dataset.
...
Never rely on a short alias like `gpt-4` rolling silently. Use the full versioned model ID:
...
Using `temperature: 0` reduces (but does not eliminate) non-determinism in judge outputs. For high-stakes rubrics, run each judge call three times and take majority vote.
...
- **Writing the rubric after looking at outputs.** Back-fitting rubric criteria to outputs you have already seen destroys the regression signal — the rubric grades your sample, not the failure taxonomy. Fix: write the rubric before building the dataset. If you back-fit, name it explicitly ("rubric written post-hoc") so the next engineer knows the baseline has observer bias baked in.
...
<Maintain trigger="LLM provider deprecates or renames a model (e.g. gpt-4 alias rolls silently)">
...
<Maintain trigger="Eval flakes on non-deterministic output (llm-rubric passes 8/10 runs, fails 2/10)">
  ...
  2. If flakiness persists at temperature 0, the rubric prompt is ambiguous — run the judge three times on the same output and check for disagreement. Rewrite the ambiguous rubric criterion with a worked PASS/FAIL example.
...
<Maintain trigger="Provider API change breaks the eval run (authentication error, endpoint 4xx/5xx)">
  ...
  3. Check promptfoo's provider adaptor for the breaking change: `npx promptfoo@latest info providers`.
  4. If the provider changed its request or response schema, bump promptfoo to the version that ships the updated adaptor.
</Maintain>

<Maintain trigger="Eval dataset rots (rows whose expected outputs no longer make sense as the product evolves)">
