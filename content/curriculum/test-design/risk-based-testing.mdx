...
```
Risk Register — Required Columns (in reading order)
────────────────────────────────────────────────────────────────────────
  Stakeholder │ Risk Description       │ Impact │ Likelihood │ Detect. │ Outcome │ Age
              │                        │ H/M/L  │ H/M/L      │ loud /  │ mitig./ │
              │                        │        │            │ quiet / │ accept/ │
              │                        │        │            │ silent  │ transfer│
────────────────────────────────────────────────────────────────────────
  learner     │ session expires        │  H     │   H        │ loud    │ mitigate│ 2024-01
              │   mid-quiz             │        │            │         │ via test│
  site owner  │ payment double-charge  │  H     │   L        │ silent  │ mitigate│ 2024-01
              │                        │        │            │         │ via test│
  learner     │ gift-wrap label        │  L     │   L        │ loud    │ accept  │ 2024-01
              │   misspelled           │        │            │         │         │

Coverage Budget (one engineer-week example)
────────────────────────────────────────────────────────────────────────
  HIGH impact + silent detect  →  exhaustive testing; exploratory charters
  HIGH impact + loud detect    →  targeted integration tests
  MED  impact + any detect     →  representative boundary cases only
  LOW  impact + any detect     →  accept or monitor; no dedicated test time
```
</Diagram>
