...
When: backend-heavy    When: frontend/API —
systems, deep logic,   logic lives in
clean module seams.    module composition.

ICE CREAM CONE          INVERTED (data)
(anti-pattern)
   _____                /____________\
  /E2E  \              /  Integration \
 /-------\            /--------------\
...
/-----------\        /__________________\
/  Integ     \
/_____________\       When: data pipelines —
     Unit             bugs live at seams
                      and production data
When: legacy/          shapes, not pure fns.
untested systems.
```
</Diagram>
