...
CI PIPELINE
──────────────────────────────────────────────────────────────────────────────
  PR PUSH          HOTFIX BRANCH      NIGHTLY              PRE-RELEASE STAGING
      │                  │                │                        │
      ▼                  ▼                ▼                        ▼
  ┌────────┐       ┌──────────┐     ┌───────────┐          ┌───────────┐
  │ SMOKE  │       │ SANITY   │     │ REGRESSION│          │   UAT     │
  │        │       │          │     │           │          │           │
  │ Wide   │       │ Narrow   │     │ Wide      │          │ User-led  │
  │ Shallow│       │ Deep     │     │ Deep      │          │ Ad-hoc    │
  │ ≤ 5min │       │ Targeted │     │ Full suite│          │ Scenarios │
  └────────┘       └──────────┘     └───────────┘          └───────────┘
  Audience:        Audience:        Audience:              Audience:
  Engineering      Engineering      Engineering            Business /
                                    + Release              Customer
  Question:        Question:        Question:              Question:
  "Does it         "Did the fix     "Has prior             "Does the
  deploy/start?"   work + hold?"    behaviour held?"       user accept?"
──────────────────────────────────────────────────────────────────────────────

Layer (per test-pyramid-and-trophy):
  Smoke      → thin E2E slice hitting real infra
  Sanity     → integration or E2E, narrow scope
  Regression → predominantly unit/integration + targeted E2E
  UAT        → exploratory, charter-led, user-operated
```
</Diagram>
